...
def main():
    args = Namespace(
        save_checkpoint=str(PROJECT_ROOT / "outputs" / "logs" / "guideddecoding" / "diode_smoke" / "checkpoints"),
        save_results=str(PROJECT_ROOT / "outputs" / "results" / "guideddecoding" / "diode_smoke"),
        num_epochs=1,
        dataset="diode",
        model="GuideDepth-S",
        weights_path=None,
